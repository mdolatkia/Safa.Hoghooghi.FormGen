        ...
        private EditAreaDataActionLog ToDataLog(DP_DataRepository item)
        {
            ...
            foreach (var property in changedPropeties)
            {
                ...
                if (property.FormulaID != 0)
                {
                    ...
                    {
                        actionLogProperty.NewValue = property.Value + " " + "محاسبه شده توسط فرمول" + " " + property.FormulaID;
                    }
                    else
                    {
                        actionLogProperty.NewValue = property.Value + " " + property.FormulaException;
                        actionLogProperty.InfoColor = InfoColor.Red;
                    }
                }
                else
                    actionLogProperty.NewValue = property.Value;
                ...
                {
                    actionLogProperty.OldValue = item.OriginalProperties.First(x => x.ColumnID == property.ColumnID).Value;
                }
